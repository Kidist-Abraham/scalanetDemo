...
val bobAddr: InetAddress = InetAddress.getByName("172.31.23.19");
 ...
 def aliceTCPPeerGroup[M](implicit scheduler: Scheduler,codec: StreamCodec[M],
      bufferInstantiator: BufferInstantiator[ByteBuffer]): TCPPeerGroup[M] = {
    val pg = new TCPPeerGroup(TCPPeerGroup.Config(aliceAddress()))
    Await.result(pg.initialize().runAsync, 10 seconds)
    pg
  }
 def bobTCPPeerGroup[M](implicit scheduler: Scheduler,codec: StreamCodec[M],
      bufferInstantiator: BufferInstantiator[ByteBuffer]): TCPPeerGroup[M] = {
    val pg = new TCPPeerGroup(TCPPeerGroup.Config(bobAddress()))
    Await.result(pg.initialize().runAsync, 10 seconds)
    pg
  }
